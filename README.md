...
See [CHEATSHEET.md](CHEATSHEET.md) for pipe, neovim, herdr, and tmux recipes,
and [PROTOCOL.md](PROTOCOL.md) for the shape: the invariants, the request
sequence, the context doctrine, the event set and the exit codes.
All code and demo artifacts in this repo were generated by a local
open-source model (llama.cpp, OpenAI-compatible endpoint). No cloud
...
# a prior run's --jsonl trace is itself context (file or pipe)
clank --jsonl -m "..." | clank -m "continue"
clank -c trace.jsonl -m "what did the assistant do?"

# several context files, concatenated in order
clank -c ctx.json -c notes.md -m "summarize both"

# map one prompt over items: one framed answer each
...
# thinking is on by default in the local templates; turn it off, or watch it
rg -l "TODO" src/ | clank --each --thinking off -m "one-line summary"
clank --show-thinking -m "why does this fail?" < error.log

# what produced a trace
clank --jsonl -m "summarize" < big.txt | jq -c 'select(.type=="run")'


# extra system directive, appended to the built-in system prompt
clank -m "Say hello." --system "Always answer with exactly: BYE"

# print the tool definitions as JSON (no model call)
clank --list-tools | jq -r '.[].function.name'

# JSONL events on stdout for jq
printf 'alpha\nbeta\n' | clank --each --jsonl -m "rate the risk 1-5" | jq -c
# schema-constrained JSON output (one request, schema enforced by the server)
...
* `--json-schema` forces the final answer to parse as JSON
  ...
  sends them together.
* Offline, no model (`cargo test`, 12 unit + 19 wire tests): `tests/wire.rs`
