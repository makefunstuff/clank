...
fn run(args: Args) -> i32 {
    ...
    let json_schema: Option<Value> = match args.json_schema {
        Some(s) => match serde_json::from_str(&s) {
            Ok(v) => Some(v),
            Err(e) => return fail(Fail::Usage(format!("invalid --json-schema: {e}"))),
        },
        None => None,
    };
    if args.list_tools {
    ...
    let tools_json = if args.no_tools {
        json!([])
    } else {
        tools::definitions()
    };
    ...
    loop {
        ...
        if result.tool_calls.is_empty() {
            if json_schema.is_some()
                && serde_json::from_str::<Value>(&result.text).is_err()
            {
                return fail(Fail::Model(
