...
`clank 0.1.0` (`~/.cargo/bin/clank`) against **oMLX 0.7.0.dev4** on
`http://127.0.0.1:8000/v1`: MacBook Pro M1 Pro, 10 cores, 16 GB unified, macOS
15.7.5. One model is resident at a time, so the server evicts and reloads on a
switch (~2 s for the 2B, ~5–7 s for the 9B, ~10 s for gemma). Pick a model per
job, not per call.
...
Sets `CLANK_BASE_URL`, `CLANK_MODEL`, `CLANK_API_KEY`. The key is read from
`~/.omlx/settings.json`; nothing is written to disk. `--model` overrides the
model per call. Quoted output below is verbatim from this endpoint, read on
2026-09-19.
...
|---|---|---|
| `Qwen3.5-9B-MLX-4bit` (5.6 GB) | anything whose output must parse or that needs a citation: review, extraction, `--each`, `--tools` | answer quickly: 6–8 s for a short answer |
...
Per-model inventiveness, context use and code correctness: *Hallucination,
context, code* below.
...
A whole tree, one process per file, `-c` for the file and `</dev/null` for
stdin:
...
list. `xargs -P2` is **2× slower** here (12.5 s vs 5.8 s over four files): the
engine batches what you send it, and each extra process pays a cold prefill.
...
Overflow is a failure, not a truncation: `--max-tokens 300` on the same command
spent 26 s and exited 1 with
`answer truncated at 300 tokens; raise it or narrow the prompt`, the partial
review on stdout. A `&&` chain stops there. Only staged hunks, one line per
hunk:
...
There is no `--commit` flag, by design: clank proposes, the shell applies.
...
Schema-constrained and gated by `jq`, 9B only:
...
Schema enforcement is per **model**, measured here with a one-property enum
schema:
...
Items are framed on stdout, so `awk '/^─── item /{…}'` splits them back;
`--jsonl` carries `i`/`of` on every event. Rewriting a list in place has the
same shape:
...
Items are text, not files:
`printf 'src/main.rs\n' | clank --each -m 'what does this file do?'` answers
*"src/main.rs is the main source file for a Rust application"*, and the model
never saw the file. If the items have to be read, the shell reads them: the `-c`
loop above, or `--each` over content you already gathered (`rg -n … | clank
--each`).
...
The model proposes the verification command, and none of it executes
automatically: `jq -er` refuses a malformed answer, `bash -n` refuses a broken
script, and the last step is you.

## A second pass over a trace
...
observers, and a general question then comes back as *"I can't answer this
request yet because I don't know what grep file to look at."*
...
The flag appends the file to the system prompt: house rules belong in the file.
Tested with a nonsense directive, `Answer only with the word: banana.`, which
answered `banana` to `what is 2+2?`, so the block is a contract, not a hint.

## Observers when nothing is piped

With nothing piped clank offers four read-only observers (`read_file`,
`list_dir`, `search`, `stat`), and every lookup goes to stderr.
...
A broad search and a README slice cannot support `src/renderer.ts:14`, and the
run still exits 0: text came back, so clank has nothing to fail. The breadcrumbs
are the only thing that catches it; below ~9B, pipe the evidence instead of
trusting the fallback. The observers are not a sandbox: they reach any path you
can read.

## Two throughput habits

```sh
clank --thinking off -m 'count from one to fifty, one number per line' | head -1   # 0.8 s, rc=141 (SIGPIPE), against 6.8 s of tokens nobody reads
```
...
single local engine only adds cold prefills. Fan out with several endpoints or
several machines.

## Four gated stages

`./demo.sh` is the reference pipeline: generate a script from a fixture,
critique it against the fixture **and the shell's own measurements**, finalize
it under `--json-schema`, then have the model propose the command that verifies
the result. Model output is syntax-checked and never executed. No run reported a
false "passed".
...
## Hallucination, context, code

Three families of probe, run twice against this endpoint. Grading is mechanical:
keyword rules for "did it deny what it cannot know", plus a real `bash` run and
...
including the trap with unrelated context piped, and all three answered the
context probes exactly, `7, E42` while the retired `3` and `E00` sat in the same
file as distractors half a sentence later. The single fabrication was a number
nobody could check: asked for `--max-rounds`'s default with nothing supplied,
the 2B answered `10`; the flag defaults to 12, and clank exited 0 because text
came back. The 9B said `None`. A local model's confident number is the one thing
its own honesty rules cannot catch.

The 2B's denial refused the premise, then did the lookup:
...
Code-generation failures were semantic: everything compiled and ran, and the
wrong answers printed the wrong thing. `bash -n` catches syntax; executing the
artifact catches the rest, and that check costs less than the model call did.
...
- 2B, C1 first run: `awk -F, '{print $2}' …` — the same header bug; second run
  it wrote `NR>1` and passed.
...
gemma-4-E4B-it is the most dependable of the three on these ten probes (6/6,
2/2, 2/2) and the least usable for anything structured: it cannot hold a
`--json-schema` and answered the wrong question under `--tools`. Qwen3.5-9B is
the one to use when output must parse or carry a citation; its code still needs
running. MiniCPM5-2B is honest about absence and 4–12× faster, and its answers
need checking: one guessed number, one self-contradiction and one header-row bug
in two passes. Each cell is n=1–2: the 2B contradicted itself on X2 (`The local
part is kept, and it's dropped by the anonymize() function.`) in the first pass
and answered cleanly in the second, so the probe tells you *what* to check, not
how often it breaks.

## Transferable design from Jev

Jev's "can't hallucinate" is not a model property: **the answer space is
closed**, the model may only pick from options you defined, and **the score
comes from a readout you control**, not from prose it wrote. Both transfer to
clank on this Mac, with one measured caveat (§6 below).

### 1. Candidates instead of a blank page

The shell finds the candidates and the model may only choose one. Measured on
three questions whose answer is a real `file:line` in this repo
...
### 1b. The same idea on a Jev-shaped fixture

18 typed decisions (6 `choice`, 6 `noul`, 6 `score`, chance = 39%), every engine
on the same rows, with the shuffled-context control from §5
(`~/Work/jev-vs-cactus/`, `out/report.md`):
...
- Closing the answer space with `--json-schema` beat the prose prompt by 6
  points and ran 3× faster (1.8 s vs 6.0 s), on the same local model, for $0.
  Hosted Jev scored the same as clank's prose arm on this fixture.
- Jev's remaining advantage is latency and a probability: 0.59 s against clank's
  1.85 s, and a distribution you can gate a threshold on, where clank answers
  carry no score at all. For 18 decisions that is 12 seconds of wall clock and
  $0.00027.
- The control separates the engines that read the state from the rest: clank
  (both arms) and Jev collapse to 11% on shuffled context, jevmlx falls to 28%,
  Cactus *rises* to 44% and never used the state.

### 2. The escape hatch

An answer space without an escape hatch forces a guess. Asked for a flag's
default value with nothing to check it against, the 2B answered `10` (it is 12)
and exited 0; asked about a file that does not exist, all three models refused,
12 traps out of 12 (`omlx-probes-*.json`). Abstention has to be a legitimate
answer, with the exit code saying whether that counts as failure *for that
call*. Correction: an earlier version of this section implied the escape hatch
might be *abused*, that a local model would take "unknown" and stop deciding.
That was a harness bug: twelve real answers were scored as abstentions because
the model emitted `{"value": true}` while the mapper expected the string
`"true"`, and oMLX does not enforce a schema's types. With the parse fixed, the
9B used the escape hatch **zero** times in 18 decisions and scored 18/18.
...
Needle's `0.9989` came with a wrong answer. The shell checks the artifact before
anything downstream trusts it. Existence is automatic, since a file that is not
there is a fabrication; the symbol check is per-claim and you write it for the
...
exist**, and the README itself quotes the old invented `src/renderer.ts:14`,
which the model then repeated back as if it were evidence. A `.rs`-only pattern
misses the first; a symbol check written for the wrong symbol misses the second.
Same shape as `jq -er` for JSON and `bash -n` for proposed commands.

### 4. One question per call, combine in code

Jev's own rule: weight the factors in your code, not in a prompt. On this box it
is also the fast path: `--each` over three commit subjects is one call (5.3 s on
the 9B), and a batched multi-field call beats a paragraph that has to be
re-parsed.

### 5. Shuffled-context control

Run the stage again with the context scrambled, no model change needed. If the
answer does not change, the stage is answering from priors.
...
Measured with exactly that control on 18 typed decisions: hosted Jev **94% →
11%**, local jevmlx **67% → 28%**, Cactus/Needle **33% → 44%**; the last one
fails the control, and 35 MB of tool-routing weights is not a decision engine.

### 6. Probability readout on this Mac

- Jev's second half, *read the distribution*, is not available through clank
  here: oMLX returns no logprobs at all (`choices[0].logprobs` is absent;
  verified on two models), so a slot readout against oMLX is impossible today.
  The only probabilities it can give are ones the model *writes*, which is the
  thing Jev exists to avoid.
- llama.cpp does return them, and one is already running on this machine
  (`:8012`, GGUF): a one-token request came back with `top_logprobs` including
  the option tokens (`A` -1.63, `Yes` -0.95). A real readout would need a second
  runtime, and clank does not surface logprobs; that second source of truth is
  what `PROTOCOL.md` rules out. If we want it, it is a `--decide` mode with its
  own wire tests (the branch plan in
  [history/decision-readout.md](history/decision-readout.md)), not a flag bolted
  onto `--json-schema`.
- Two local Jev-shaped engines exist, and the trained one wins. `jevmlx` (a
  general MLX model plus a slot readout) ran 18 typed decisions at 67% with a
  ...
  control at an **83 ms median**, no network and no credentials. Both are
  sidecars to clank rather than replacements for it, and `clank-jev` is the
  sidecar that talks to either (`--provider kev`, `--provider openrouter`).

### 7. What does not transfer

A prompted JSON schema is not a grammar: on this oMLX build only
`Qwen3.5-9B-MLX-4bit` enforced `--json-schema`, gemma returned a fenced block
and the 2B answered in prose. The closed answer space holds only where it has
been measured, and on every other model the mechanical gate in §3 does the work.
...
- **`--json-schema` on the 2B and gemma.** Fenced or prose answers, `exit 1`.
  Use the 9B, or check the shape yourself with `jq -er`.
- **`--tools` below ~9B.** One correct answer, one invention, one misread
  question in the runs here.
- **`--each` over bare paths.** Items are text; the model cannot read them.
- **`xargs -P` against one engine.** Slower than serial, and the answers
  interleave.
- **`--thinking` on gemma-4-E4B.** No delta, since that quantisation has no
  reasoning mode. Real 1.8× on the 2B, small win on the 9B.
- **`Bonsai-2-27B-CRACK-1.75bit-JANG`.** Listed by `/v1/models`, unloadable
  here; clank passes the server's 409 through verbatim, 402 parameter names
  included.
- **A `while read` loop without `</dev/null`.** The first call consumes the
  list.
